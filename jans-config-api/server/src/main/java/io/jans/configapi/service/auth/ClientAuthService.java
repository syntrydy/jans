...
public class ClientAuthService {
    ...
    public List<ClientAuthorization> getClientAuthorizations(String clientId) {
        logger.error(" Authorizations details to be fetched bassed on clientId:{} ", clientId);
        ...
        List<ClientAuthorization> clientAuthorizations = persistenceEntryManager.findEntries(getClientAuthorizationDn(null), ClientAuthorization.class,
                clientIdFilter);
        logger.error(" Authorizations details to be fetched for clientAuthorizations:{} ", clientAuthorizations);
    ...
    public Map<Client, Set<Scope>> getUserClientAuthorizationData(String userId) {
        ...
        if (clientAuthorizations==null || clientAuthorizations.isEmpty()) return Collections.emptyMap();
        
        //Obtain client ids from all this user's client authorizations
        Set<String> clientIds = clientAuthorizations.stream().map(ClientAuthorization::getClientId).collect(Collectors.toSet());
        logger.error(" Authorizations clientIds:{} ", clientIds);
        Filter[] filters = clientIds.stream().map(id -> Filter.createEqualityFilter("inum", id))
                .collect(Collectors.toList()).toArray(new Filter[]{});
        List<Client> clients = persistenceEntryManager.findEntries(clientService.getDnForClient(null), Client.class, Filter.createORFilter(filters));
        logger.error(" Authorizations clients:{} ", clients);
        
        List<Scope> scopes = getScopeList(clientAuthorizations);
        logger.error(" Authorizations scopes:{} ", scopes);
    ...
    private List<Scope> getScopeList(List<ClientAuthorization> clientAuthorizations) {
        logger.error("Client authorizations for clientAuthorizations {}", clientAuthorizations);
        Set<String> scopeIds = new HashSet<>();
        for (ClientAuthorization auth : clientAuthorizations) {
            logger.error("Client authorizations for auth.getClientId():{}, auth.getScopes():{}", auth.getClientId(), auth.getScopes());
            if (auth.getScopes()!=null && auth.getScopes().length>0) {
                Arrays.asList(auth.getScopes()).stream().filter(scopeIds::add);
            }
        }
        logger.error("scopeIds:{}", scopeIds);
